---
title: "STAT 33B Homework 6"
date: "Apr 11, 2021"
author: "Won Shil Park (3033452021)"
output: pdf_document
---

This homework is due __Apr 23, 2021__ by 11:59pm PT.
...
```
The total time to run 1,000,000 (1e6) samples took 38650 ms, since it took too much time and 
since 10,000 (1e4) samples takes around 30 ms, I will be sticking with only generating 100,000 (1e5) 
samples since that only takes 820 ms. 
Based on the profile, the majority of the time is spent on the 'chain = c(chain, x)'.
We can change this by changing this portion along with the value 'chain' and preallocating.
...
```
Due to the corrections, the timing for generating 100,000 samples only took 430 ms, lessening the time by almost half.
...
profvis({
  sample_markov = function(n, p = c(0.6, 0.7), init = 0) {
    ...
    for (i in 1:n) {
      # Check the previous state, chain[i].
      if (chain[i] == 0) {
        # Get the probabilities for {0 to 0} and {0 to 1}.
        prob = c(1 - p[1], p[1])
        # Randomly sample new state using the probabilities.
        ...
        # Get the probabilities for {1 to 0} and {1 to 1}.
        prob = c(1 - p[2], p[2])
        # Randomly sample new state using the probabilities.
        x = sample(c(0, 1), 1, prob = prob)
        chain[i + 1] = x
...
```
By changing all "A" to 0 and all "B" to 1, the timing changed to 360 ms, which is less but not as much of a difference.
Now, there is less codes that time is being spent on and the now edited line of code 'x = sample(c(0, 1), 1, prob = prob)'
is taking less of time.
...
profvis({
  sample_markov = function(n, p = c(0.6, 0.7), init = 0) {
    ...
    for (i in 1:n) {
      # Check the previous state, chain[i].
      if (chain[i] == 0) {
        # Get the probabilities (simplified) from {0, 1}
        chain[i + 1] = rbinom(1, 1, prob = p[1])
        
      } else {
        # Get the probabilities (simplified) from {1, 1}
...
```
Now, the code is only spending time on those two added codes with the new function 'rbinom'.
Less time is spent on the code chunk and based on the profile, about half the time was saved due to the new modifications.
Time: 190 ms.
...
profvis({
  sample_markov = function(n, p = c(0.6, 0.7), init = 0) {
    ...
    # generate random deviates using n, doesn't need parameters since default
    samp = runif(n)
    ...
    for (i in 1:n) {
      ...
      if (chain[i] == 0) {
        # Get the probabilities (simplified) from {0, 1}
        chain[i + 1] = as.integer(samp[i] < p[1])
        
      } else {
        # Get the probabilities (simplified) from {1, 1}
        chain[i + 1] = as.integer(samp[i] < p[2])
      }
    }
...
```
By coming the step 1 value and using as.integer, it only takes 30 ms to finish this code chunk. 
In the beginning of this homework's exercise, the chunk took 820 ms. It has now considerably lessened by halves. It's now ~27x faster. 
